...
import { ChevronRight, ArrowLeft, Mail,  ShieldCheck,  } from "lucide-react";
...
function SecurityItem({
    ...
    return (
        <div className="flex items-center justify-between">
            ...
            {!checked && <ChevronRight className="w-5 h-5 text-neutral-500" />}
        </div>
    );
}

function LinkItem({ label, value, link }: { label: string; value?: string, link?: string }) {
    return (
        <Link href={`${link}`}>
